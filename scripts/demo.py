...
def main(config, args):
    ...
    for i, imga_name in enumerate(glob.glob(args.source_dir+'/*/female/*')):
        if '.jpg' in imga_name or '.png' in imga_name or '.JPG' in imga_name:
          print(imga_name)
          for j, imgb_name in enumerate(glob.glob(args.reference_dir+'/*.png')):
            ...
            save_path = imga_name.replace('selected', 'output').split('.')[0]+'-'+os.path.basename(imgb_name).split('.')[0]+'.png'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_path", type=str, default='/content/output', help="path to save model")
